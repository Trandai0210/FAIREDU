...
if dataset_name == 'adult':
    dataset_orig = pd.read_csv('../datasets/adult.data.csv')
    ...
    # Load dataset
    dataset_orig = pd.read_csv('../datasets/compas-scores-two-years.csv')
...
elif dataset_name == 'student_oulad':
    # Load dataset
    ...
    # Drop categorical features
    dataset_orig = dataset_orig.drop(['PlaceofBirth','GradeID','SectionID','Topic','Semester','Relation','StudentAbsenceDays'],axis=1)
...
DI_b = {
    ...
    for model in ['logistic', 'random_forest', 'boost', 'decision_tree']
}
        ...
        try: 
            for k in range(iterations):
                ...
                for clf_name, clf_i in clfs.items():
                    ...
                    privileged_groups = {
                        attr: dataset_pred.privileged_protected_attributes[dataset_t.protected_attribute_names.index(attr)][0] for
                        attr in attrs}
                    unprivileged_groups = {
                            attr: dataset_pred.unprivileged_protected_attributes[dataset_t.protected_attribute_names.index(attr)][0] for
                            attr in attrs}

                    class_metrics = {
                            attr: ClassificationMetric(dataset_t, dataset_pred, unprivileged_groups=[{attr: unprivileged_groups[attr]}],
                                                    privileged_groups=[{attr: privileged_groups[attr]}]) for attr in attrs}
